...
const createTranslationsSlice = createSlice({
  ...
  extraReducers: builder => {
    builder.addCase(modules_openai_audio_transcription_create.pending, (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending";
      }
    }).addCase(modules_openai_audio_transcription_create.fulfilled, (state, action) => {
      if (state.api.loading === "pending") {
        state.entities.push(action.payload);
        state.api.loading = "idle";
      }
    }).addCase(modules_openai_audio_transcription_create.rejected, (state, action) => {
      if (state.api.loading === "pending") {
